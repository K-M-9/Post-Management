...
class GetFavoritePostsUseCaseTest {
    ...
    private val useCase = GetFavoritePostsUseCase(repository)


    @Test
    fun `GIVEN repository returns favorite posts flow WHEN invoke is called THEN return same flow`() =
        runTest {
            // Given
            val expectedPagingData = PagingData.from(listOf<Post>())
            val expectedFlow = flowOf(expectedPagingData)
            every { repository.getFavoritePosts() } returns expectedFlow
            ...
            verify(exactly = 1) { repository.getFavoritePosts() }
        }
}