...
class GetMyPostsUseCaseTest {
    ...
    private val useCase = GetMyPostsUseCase(repository)

    @Test
    fun `GIVEN repository returns my posts flow WHEN invoke is called THEN return same flow`() =
        runTest {
            // Given
            val expectedPagingData = PagingData.from(listOf<Post>())
            val expectedFlow = flowOf(expectedPagingData)
            every { repository.getMyPosts() } returns expectedFlow
            ...
            verify(exactly = 1) { repository.getMyPosts() }
        }
}